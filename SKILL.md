...
- `"start autopilot don't sleep"` / `"start autopilot stay awake"` → `autopilot start --keep-awake`
...
| `autopilot start --keep-awake` | Same as `autopilot start` but blocks idle sleep for as long as autopilot is ticking. Each tick renews a short `caffeinate -i` lease (3x the tick interval); the lease expires by itself if the daemon crashes or is unloaded, and `autopilot stop` kills it immediately. | `… autopilot start --keep-awake` |
| `autopilot stop` | Unload autopilot plist + drop the sleep block. Pool + queue stay. scan-bg keeps running unless you also `stop-bg`. | `… autopilot stop` |
...
| System sleep / lid close | **launchd PAUSES.** Ticks stop. Time spent asleep is LOST (no backfill). | Use `autopilot start --keep-awake` — a per-tick `caffeinate -i` lease that only lives while autopilot runs |
